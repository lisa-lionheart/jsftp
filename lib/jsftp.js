...
Ftp.prototype.getPasvSocket = function(callback) {
  ...
  this.execute("pasv", function(err, res) {
    ...
    getPasvPort(res.text, function(err, res) {
      ...
      var socket = Net.createConnection(res.port, res.host, function(e){
        callback(e, socket);
      });
      socket.setTimeout(timeout || TIMEOUT);
      socket.on('timepout', callback);

      //Server may jave not correctly reported host, so try the one we connected with
      if(res.host !== self.host) {
        var socket2 = Net.createConnection(res.port, self.host, function(e){
          callback(e, socket2);
        });
        socket2.setTimeout(timeout || TIMEOUT);
        socket2.on('timepout', callback);
      }
    });
  });
};
